<template>
			<!-- 设计费 附加调整 模态框 -->
	<view class="cu-modal" @touchmove.stop.prevent="moveHandle" :class="showModalName== modalName ?'show':''">
		<view class="cu-dialog">
			<view class="cu-bar bg-white justify-end">
				<view class="content">节能评估费调整系数</view>
				<view class="action" @tap="hideModal">
					<text class="cuIcon-close text-red"></text>
				</view>
			</view>
			<view class="padding-xl border border-blue m-1">
				<view class="dlf-group lt">
					<view class="w-100 dlf-li font lt border-b" 
					v-for="(item, index) in tzxsList" :key="index" @tap="assignment" :data-val="item.xs">
						<view class="lt" style="width: 87%; white-space: pre-wrap;">
							{{item.content}}
						</view>
						<span class="spbtn text-white px-1 bg-blue rt" >
							{{item.val}}
						</span>
					</view>
				</view>
			</view>
		</view>
	</view>
</template>

<script>
	import {modalMixin} from "@/common/base/modalMixin"
	
	export default {
		mixins: [modalMixin],
		props: {
			showModalName: {
				type: String,
				default: null
			}, // 弹框可见标志
		},
		data() {
			return {
				modalName:'jnpgfHu_tzxs',
				tzxsList:[
					{content:"钢铁、有色金属、石化、化工、化纤、医药、能源、燃气热力",val:"1.2"},
					{content:"资源综合利用、生态与环保、海洋工程、通讯信息、广播电影电视",val:"0.8"},
					{content:"城市道路、公路、铁路、港口航道河道、风景园林、环境卫生、农业、林业、国土资源 ",val:"0.6"},
					{content:"其他行业（包括机械、电子、轻工、纺织、建材、给排水、水利工程、城市轨道交通、民航、建筑等）",val:"1.0"}
				]
			}
		},
	}
</script>

<style >
	
	
</style>