		...
		<view class="cu-dialog">
			...
			<view class="inputBox">
				<view class="boxtop flexbox">
					<view class="toptitle">测绘面积:</view>
					<input type="text" v-model="chmj">
					<uni-tag text="k㎡" type="defult"></uni-tag>
				</view>
				<view class="boxbottom flexbox">
					<view class="toptitle">单幅图面积:</view>
					<input type="text" v-model="dftmj">
					<uni-tag text="k㎡" type="defult"></uni-tag>
				</view>
			</view>
			<view class="warp">
...
<script>
	...
	export default {
		...
		methods: {
			computedData(){
				let result = this.chmj/this.dftmj*1.3
				if(result){
					result = result.toFixed(2)
				}else{
					result = 0
				}
				this.$emit('getData',{type:xmjxs,val:result})
			}
		}
	}
</script>
