<template>
	<!-- 设计费 复杂调整 模态框 -->
	<view class="cu-modal" @touchmove.stop.prevent="moveHandle" :class="showModalName == modalName ? 'show' : ''">
		<view class="cu-dialog">
			<view class="cu-bar bg-white justify-end">
				<view class="content">设计收费专业调整系数</view>
				<view class="action" @tap="hideModal"><text class="cuIcon-close text-red"></text></view>
			</view>
			<view class="padding-xl border border-blue m-1">
				<view class="dlf-group lt">
					<view class="w-100 dlf-li font lt border-b" 
					v-for="(item, index) in fjcdList" :key="index" @tap="assignment" :data-val="item.xs">
						<view class="lt" style="width: 87%; white-space: pre-wrap;">
							{{item.value}}
						</view>
						<span class="spbtn text-white px-1 bg-blue rt" >
							{{item.xs}}
						</span>
					</view>
				</view>
				<view class="lt font p-1 text-left" style="white-space: pre-wrap;">
					<view>注:</view>
						<view>附加调整系数为两个或两个以上的, 附加调整系数相加。</view>
				</view>
			</view>
		</view>
	</view>
</template>

<script>
import { modalMixin } from '@/common/base/modalMixin';
export default {
	mixins: [modalMixin],
	props: {
		showModalName: {
			type: String,
			default: null
		} // 弹框可见标志
	},
	data() {
		return {
			modalName: 'sjfGui_fjtz',
			fjxs: '',
			fjcdList: [
				{value:"改扩建和技术改造建设项目 （系数范围：0.1 - 0.4）",xs:"0.3"},
				{value:"古建筑仿古建筑、保护性建筑等（系数范围：0.3 - 0.6）",xs:"0.45"},
				{value:"智能建筑弱电系统设计，以弱电系统的设计概算为计费额",xs:"0.3"},
				{value:"室内装修设计,以室内装修的设计概算为计费额",xs:"0.5"},
				{value:"特殊声学装修设计，以声学装修的设计概算为计费额",xs:"1.0"},
			]
		};
	},
	methods: {
		
	},
	

};
</script>

<style>

</style>
